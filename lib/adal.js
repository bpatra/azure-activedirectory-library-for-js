...
AuthenticationContext = function (config) {
    ...
    */
    ...
    this.config.usePopupForLogin = typeof this.config !== "undefined";
...
AuthenticationContext.prototype.login = function () {
    ...
    var urlNavigate = this._getNavigateUrl('id_token', null,this.config.usePopupForLogin) + '&nonce=' + encodeURIComponent(this._idTokenNonce);
...
AuthenticationContext.prototype.promptUser = function (urlNavigate) {
    ...
    if (urlNavigate) {
        ...
        if(this.config.usePopupForLogin){
...
AuthenticationContext.prototype._getNavigateUrl = function (responseType, resource,usePopupLoginUri) {
    var tenant = 'common';
    if (this.config.tenant) {
        tenant = this.config.tenant;
    }
    ...
    var urlNavigate = this.instance + tenant + '/oauth2/authorize' + this._serialize(responseType, this.config, resource, usePopupLoginUri) + this._addClientId();
...
AuthenticationContext.prototype._serialize = function (responseType, obj, resource,usePopupLoginUri) {
    var str = [];
    if (obj !== null) {
        ...
        if(usePopupLoginUri === true){
             str.push('redirect_uri=' + encodeURIComponent(obj.popupConfigUri));
        }
        else{
