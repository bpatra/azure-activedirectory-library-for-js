...
AuthenticationContext.prototype.promptUser = function (urlNavigate) {
    if (urlNavigate) {
        ...
        if(inIframe()){
            ...
            var popup = window.open(urlNavigate,'Auth window', "height=500,width=500");;
            var interval;;
            var pollChildWindow = function () {
                var locationHref = '';
                var isAboutBlank = true;
                try{
                    locationHref = popup.location.href;
                    isAboutBlank = popup.location.origin == "null";
                }catch(ex)
                {
                ...
                else if (!isAboutBlank && locationHref !== urlNavigate) {
